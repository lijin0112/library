...
function initProvince(provinceName) {
  var initjScrollPane = function () {
    $('div.stores > div.scrollpane').jScrollPane({
      animateTo: true,
      animateInterval: 50,
      animateStep: 5,
      showArrows: true,
...
function loadingProvinceHtml(provinceName, callback) {
  ...
  $.ajax({
    ...
    type: "post",
    url: "/cn/sale/" + provinceName + "/index.html",
    beforeSend: function () {
      ...
      $items.find("li").each(function (i, data) {
        $(data).hover(
          function () {
            ...
            $("div.network div.smallstoredetail").show().css({
              "left": offsetleft + "px",
              "top": offsettop + "px"
            }).find(".bg").html("<div class='loading'></div>");
...
function fillStoreDetail(address, item) {
  ...
  $.get(
    ...
    function (data) {
      ...
      $storedetail.append($(data).siblings(".googlemap"))//http://ditu.google.cn/maps?hl=zh-CN&q=
      $storedetail.find(".googlemap a").attr({"href": "http://map.baidu.com/?newmap=1&l=18&tn=B_NORMAL_MAP&s=s%26wd%3D" + encodeURI($storedetail.find(".googlemap a").attr("address")) + "&um=1&ie=UTF-8&sa=N&tab=wl"})
...
function fillSmallStoreDetail(address, item) {
  ...
  $.get(
    ...
    function (data) {
      ...
      $smallstoredetailbg.find(".googlemap a").attr({"href": "http://map.baidu.com/?newmap=1&l=18&tn=B_NORMAL_MAP&s=s%26wd%3D" + encodeURI($smallstoredetailbg.find(".googlemap a").attr("address")) + "&um=1&ie=UTF-8&sa=N&tab=wl"})
...
function swfLoaded() {
  $.get(
    ...
    function (data) {
      initProvince("shanghai");
      callselectSity("shanghai")
    }
  )
}
//JS TO FLASH
...
// window.document.write("\<" + "script src=\"" + window.location.origin  + "/cn/resource/js/jquery.tmpl.js\" type=\"text/javascript\" charset=\"utf-8\"><\/script>")
// window.document.write("\<" + "script src=\"" + window.location.origin  + "/cn/resource/js/jquery.tmplPlus.js\" type=\"text/javascript\" charset=\"utf-8\"><\/script>")

//window.document.write("\<" + "script src=\"" + "/web/static/oppo/js/cn/jquery.tmpl.js\" type=\"text/javascript\" charset=\"utf-8\"><\/script>");
//window.document.write("\<" + "script src=\"" + "/web/static/oppo/js/cn/jquery.tmplPlus.js\" type=\"text/javascript\" charset=\"utf-8\"><\/script>");

window.document.write("\<" + "script src=\"http://api.map.baidu.com/api?v=1.3\" type=\"text/javascript\" charset=\"utf-8\"><\/script>")
...
function loadJson(option) {
  return $.ajax({
    ...
    success: function (json) {
      if ($.isFunction(option.successCallBack))
        option.successCallBack(json)
    },
    beforeSend: function (xhr) { //console.log("beforeSend")
      // xhr.setRequestHeader('Content-Type', 'application/json');
      // //web<>guzuqiang@foxmail.com<>AEDE3264<>94560<>0<>0
      // xhr.setRequestHeader('Authorization', getCookie("Authorization"));
      if ($.isFunction(option.beforeSendCallBack))
        option.beforeSendCallBack(xhr)
    },
    complete: function (xhr, textStatus) {//console.log("complete")
      if ($.isFunction(option.completeCallBack))
        option.completeCallBack(xhr, textStatus)
    },
    error: function (xhr, textStatus, errorThrown) {
      //console.log(textStatus + "  " + errorThrown)
      if ($.isFunction(option.errorCallBack))
...
//读入商铺列表
var loadStores = function (data, successCallBack) {
  return loadJson({
    //url : "/cn/sale/" + data.province + "/index.html",//"/cn/sale/list.html",//"/cn/resource/test.js",
    url: "/home/getstore?type=store",//"/cn/sale/list.html",//"/cn/resource/test.js",
    type: "get",
    data: data,
    successCallBack: successCallBack
  })
}
var randerStoreList = function (data) {
  /*$(".storelist .scrollpane").removeClass("loading");
   $.tmpl('storeListItemTemplate', data,{
   dataArrayIndex: function (item) {
   return $.inArray(item, data);
   }
   })
   .appendTo(".storelist .scrollpane ul");
   initScrollPane();
   */
  alert(data);
}
//查询商店类型是否包含
var isStoreType = function (storeTypes, storeType) {
  // console.log(storeTypes.length)
  if (storeTypes.length == 0) return true;//商店类型列表为空时，查询全部

  for (var i in storeTypes) {
    if (storeTypes[i] == storeType) {
      return true;
    }
  }
  return false;
...
//执行搜索动作
var searchStores = function (data) {
//	var $searchBox = $(".searchbox");
//	$searchBox.animate({"margin-top":"0px"},250);//$.tmpl('TableList.listTemplate', data
//	$searchBox.find(".line").animate({"opacity":"0"},250,function(){$(this).hide()});
//	$(".storelist .scrollpane ul").empty();
//	var itemListAPI = $(".storelist .scrollpane").data('jsp');
//	if (itemListAPI) {itemListAPI.destroy();}
  $(".storelist").html('');
  setTimeout(function () {
    //$(".storelist").show().find(".scrollpane").addClass("loading").find("ul").empty();
    loadStores(data, function (result) {
      //按照条件过滤
      var tempData = [];
      var html = '';
      $.each(result, function (index, item) {
        //console.log(item.name + "&&" + item.type)
        // console.log(data.keyword + "&&" + data.type.join(","))
        // console.log(item.name.indexOf(data.keyword) >= 0 && isStoreType(data.type, item.type))
        // $.inArray(item.type,data.type) > -1
        //if (item.name.indexOf(data.keyword) >= 0 && isStoreType(data.type, item.type)){
        //		tempData.push(item);
        //}
        //	alter( item );
        if (item.level == 1) {
          html += '<dl class="dt_title dt_title_good"><a href="#" title="卓越经销商"><dt>' + (index + 1) + '</dt> <dd><p>' + item.name + '</p><p>' + item.address + '</p></dd></a></dl>';
        }
        else {
          html += '<dl class="dt_title"><dt>' + (index + 1) + '</dt> <dd><p>' + item.name + '</p><p>' + item.address + '</p></dd></dl>';
        }
      })
      $(".storelist").html(html);
      $(".storelist").show();
      //randerStoreList(tempData)
    })
  }, 300)
}
...
var loadProvince = function () {
  var $provinceList = $(".provincelist ul").empty();
  $.each(flashvars.cityArrStr, function (index, item) {
    var $li = $("<li></li>");
    $li.html(item.cn_name).data("province", item).attr("en_name", item.en_name)
    $provinceList.append($li);
  })
  $provinceList.find("li:last").css({"border-bottom": "0px none"})
}

//隐藏全部
var hiddenAllBox = function () {
  $(".changecontent .map").hide();//.empty()
  $(".changecontent .storemap").hide();//.empty()
  $(".changecontent .storepicture").hide();//.empty()
}
//显示大地图
var showBigMap = function () {
  $(".changecontent .map").show();//.empty()
  $(".changecontent .storemap").hide();//.empty()
  $(".changecontent .storepicture").hide();//.empty()
  $(".storelist li").removeClass("selected")
}

//加载图片
var loadPicture = function (data) {
  hiddenAllBox();
  $(".changecontent .storepicture").show();
  $(".changecontent .storepicture h3").html(data.name);
  $(".changecontent .storepicture .picturecount").html("1 / " + data.images.length);//console.log($(".changecontent .storepicture .picturelist").width() * data.images.length)
  $(".changecontent .storepicture .picturelist ul").empty().width($(".changecontent .storepicture .picturelist").width() * data.images.length);

  $.tmpl("pictureItemTemplate", data.images)
    .appendTo(".changecontent .storepicture .picturelist ul")
}

//加载地图
var loadBaiduMap = function (data) {
  console.log(data)
  hiddenAllBox();
  $(".changecontent .storemap").show();
  $(".changecontent .storemap h3").html(data.name);
  $(".changecontent .storemap .address").html(data.address);
  ...
  var marker;
  var infoWindow = new BMap.InfoWindow($.tmpl("baiduMapTemplate", data).htmlCode());  // 创建信息窗口对象
  // // 创建点坐标
  // var point = new BMap.Point($map.attr("data-location-longitude"), $map.attr("data-location-latitude"));//
  // 创建地址解析器实例
  var myGeo = new BMap.Geocoder();
  myGeo.getPoint(data.map_address, function (point) {
    if (point) {
...
var storeTypeToCss = function (type) {
  var typeCss = "";
  switch (type) {
    case "大卖场":
    {
      typeCss = "hypermarkets";
      break;
    }
    case "旗舰店":
    {
      typeCss = "flagship_store";
      break;
    }
    case "展示厅":
    {
...
var pictureItemTemplate = "<li>" +
  "<img src=\"${$data}\">" +
  "</li>"
var baiduMapTemplate = "<div class=\"map_massage\"><ul>" +
  "<li class=\"title\">${name}</li>" +
  "<li>${address}</li>" + //"<li>官网：<a href=\"{5}\" class=\"txt14\">{2}</a></li>" + //<a href=\"http://{2}\" class=\"icon_homepage\"></a>
    //"<li>${intro}</li>" +
  "<li>电话：${tel} 传真：${fax}</li>" +
  "</ul></div>";
var storeListItemTemplate = "        				<li>" +
  "        					<div class=\"number\">${$item.dataArrayIndex($item.data) + 1}</div>" +
  "        					<div class=\"info\">" +
  "        						<div class=\"header\">${name}<span class=\"icon ${storeTypeToCss(type)}\">${type}</span></div>" +
  "        						<div class=\"text\">地址：${address}</div>" +
    //"        						<div class=\"text\">${intro}</div>" +
  "        						<div class=\"text\">电话：${tel}</div>" +
  "        						<div class=\"text\">传真：${fax}</div>" +
  "        						<div class=\"links\">" +
  "        							<a href=\"#\" class=\"map\">查看地图</a>{{if images.length > 0}}<a href=\"#\" class=\"picture\">图片查看</a>{{/if}}" +
  "        						</div>" +
  "        					</div>" +
  "        					<div class=\"clear\"></div>" +
  "        				</li>"
...
var mapSendToProvinceList = function (province) {
  var $province = $(".provincelist ul li[en_name='" + province + "']");
  if ($province.length > 0) {
    $province.trigger("click");
    var data = {};
    data.province = $(".province").data("province") ? $(".province").data("province").en_name : "";
    data.keyword = "";
    data.type = [];
    $("[name='storeType']").attr("checked", false);
    $("input.keyword").val("");
    searchStores(data);
    //$(".searchbox .searchbutton").trigger("click");
  }
}

var searchBoxSendToMap = function (provinceValue) {
  var _province = province[provinceValue];
  //console.log(_province);console.log(provinceValue);
  if (_province) {
    // 高亮 选中的地图块
    // eg：_province -> Object {cn_name: "北京", en_name: "beijing", label: B, path: B}
    //    eve("province.highlight", "self", _province);  北京地图块变色。
    eve("province.highlight", "self", _province);
    // _province.path.events[0].f();
  }
}
...
$(function () {
  ...
  var paper = Raphael("map_container", 632, 540);
  var attrTest = {"fill": "#000", "stroke": "#fff", "opacity": "1", "stroke-width": 0};
  var attr = {"fill": "#EFEFEF", "stroke": "#fff", "opacity": "1", "stroke-width": 0};
  var attrHover = {"fill": "#0981dd", "stroke": "#fff", "opacity": "1", "stroke-width": 0};
  var transformValue = "t15,5", scaleValue = [.6, .6, 0, 0];
  var labelAttr = {fill: "#000", "font-size": "12px", opacity: 1};
  var font = paper.getFont("yahei");
  var showLabel = function (x, y) {
    return paper.text(x, y, this.cn_name).attr(labelAttr);
  }
  ...
  for (var item in province) {
    ...
    province[item].path.transform(transformValue);
    province[item].path.scale(scaleValue);
  }

  eve.on("province.highlight", function (el) {
    unselectedProvince();
    el.path.attr(attrHover);
  })
  ...
  var checkExternalClick = function (event) {
    ...
    if ($target.hasClass("provincelist")) return false;
    if ($target.parents("div.province").length == 0 && $target.parents("div.provincelist").length == 0) {
      $("div.provincelist").hide();
    }
  }
  ...
  $.template('pictureItemTemplate', pictureItemTemplate);
  $.template('baiduMapTemplate', baiduMapTemplate);
  $.template('storeListItemTemplate', storeListItemTemplate);
  loadProvince();
  var $searchBox = $(".searchbox");
  $(".province").delegate("a", "click", function () {
    $(".provincelist").show();
    return false;
  })
  $(".provincelist").delegate("li", "mouseenter", function (e) {
    $(this).addClass("current");
  })
  $(".provincelist").delegate("li", "mouseleave", function (e) {
    $(this).removeClass("current");
  })
  $(".provincelist").delegate("li", "click", function () {
    var data = $(this).data("province")
    $(this).parents(".provincelist").hide();
    $(".province span").html(data.cn_name)
    $(".province").data("province", data);
  })

  //搜索按钮点击
  $("div.keyword").delegate(".searchbutton", "click", function () {
    var data = {}, storeTypes = [];
    $("[name='storeType']:checked").each(function (index, item) {
      storeTypes.push($(item).val());
    })
    data.province = $(".province").data("province") ? $(".province").data("province").en_name : "";
    ...
    data.type = storeTypes;//.join(",");
    // data.successCallBack = function(result){randerStoreList(result)}
    searchStores(data);
    showBigMap();
    searchBoxSendToMap($(".province").data("province").en_name);
    // $.when($searchBox.animate({"margin-top":"0px"},250),$searchBox.find(".line").animate({"opacity":"0"},250,function(){$(this).hide()})).done(function(){
    // 	$(".storelist").show().find(".scrollpane").addClass("loading").find("ul").empty();
    // 	loadStores(data).done(function(result){
    // 		randerStoreList(result)
    // 	})//.always(function(a){randerStoreList(eval("(" + a.responseText + ")"))})
    // })
    return false;
  })
  $(".storelist").delegate("li", "mouseleave", function () {
    $(this).removeClass("current");
  })
  $(".storelist").delegate("li", "mouseenter", function () {
    $(this).addClass("current");
  })
  //地图查看
  $(".storelist").delegate("a.map", "click", function () {
  ...
  //返回大地图
  $(".storelist").delegate("a.return", "click", function () {
    showBigMap()
    return false;
  })

  //图片查看
  $(".storelist").delegate("a.picture", "click", function () {
    var data = $.tmplItem(this).data;
    $(".picturecount").data("pageNo", 0)
    changePage(0);
    loadPicture(data);
    $(this).parents("li").addClass("selected").siblings().removeClass("selected");
    return false;
  })
  $(".storepicture").delegate(".picturelist", "mouseleave", function () {
    $(".storepicture .next").hide();
    $(".storepicture .prev").hide()
  })
  $(".storepicture").delegate(".picturelist", "mousemove", function (e) {
    var percentage = (e.pageX - $(this).offset().left) / $(this).width() * 100;
    if (percentage > 49) {
      if ($(".storepicture .next").is(":hidden"))
        $(".storepicture .next").show()
      if (!$(".storepicture .prev").is(":hidden"))
        $(".storepicture .prev").hide()
    }
    else {
      if ($(".storepicture .prev").is(":hidden"))
        $(".storepicture .prev").show()
      if (!$(".storepicture .next").is(":hidden"))
        $(".storepicture .next").hide()
    }
  })
  //显示页数
  var showCurrentPage = function (pageNo) {
    $(".picturecount").html((pageNo + 1) + " / " + $(".picturelist ul li").length).data("pageNo", pageNo)
  }
  //切换页面
  var changePage = function (pageNo) {
    var moveWidth = pageNo * 602;
    $(".picturelist ul").animate({"margin-left": -moveWidth + "px"}, 600, function () {
      showCurrentPage(pageNo);
    })
  }
  $(".storepicture").delegate(".prev,.next", "click", function () {
    var $this = $(this);
    var pageNo = $(".picturecount").data("pageNo") ? $(".picturecount").data("pageNo") : 0;
    if ($this.hasClass("prev")) {
      ...
      pageNo = parseInt(pageNo + 1);
      pageNo = pageNo > $(".picturelist ul li").length - 1 ? $(".picturelist ul li").length - 1 : pageNo;
    }
    $(".picturecount").data("pageNo", pageNo)
    changePage(pageNo);
    return false;
  })

  //返回大地图
  $(".changecontent").delegate("a.return", "click", function () {
    showBigMap()
    return false;
  })

  var enName = window.location.hash.length > 0 ? window.location.hash.slice(1) : null;//console.log(enName)
  if (enName) {
    $(".provincelist li").filter("[en_name='" + enName + "']").trigger("click");
    setTimeout(function () {
      $(".stores .searchbutton").trigger("click")
    })
  }
