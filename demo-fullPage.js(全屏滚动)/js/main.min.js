...
function PxLoaderVideo(a, b, c, d) {
    ...
    void 0 !== d && (this.vid.crossOrigin = d),
    this.tags = b,
    this.priority = c;
    var h = function() {
    ...
    k = function() {
        e.unbind("load", i),
        e.unbind(e.readyEventName, h),
        e.unbind("error", j)
    };
    this.start = function(b) {
        f = b,
        e.bind("load", i),
        e.bind(e.readyEventName, h),
        e.bind("error", j),
        e.bind("suspend", i),
        e.vid.src = a,
        e.vid.load()
    ...
    this.onTimeout = function() {
        k(),
        4 !== e.vid.readyState ? f.onLoad(e) : f.onTimeout(e)
    },
    this.getName = function() {
        return a
    },
...
function(a) {
    function b(a) {
        ...
        var i = function(a) {
            a = h(a);
            var b = function(b) {
                for (var c = b.resource,
                d = 1 / 0,
                e = 0; e < c.tags.length; e++) for (var f = 0; f < Math.min(a.length, d) && !(c.tags.all[e] === a[f] && d > f && (d = f, 0 === d)) && 0 !== d; f++);
                return d
            };
            return function(a, c) {
                var d = b(a),
        ...
        var j = function() {
            for (var b = !1,
            c = Date.now() - f, e = c >= a.noProgressTimeout, h = c >= a.loggingDelay, i = 0, k = d.length; k > i; i++) {
                var l = d[i];
        ...
        this.isBusy = function() {
            for (var a = 0,
            b = d.length; b > a; a++) if (d[a].status === g.QUEUED || d[a].status === g.WAITING) return ! 0;
            return ! 1
        };
        var k = function(a, b) {
            ...
            if (null != m && m.status === g.WAITING) for (m.status = b, f = Date.now(), i = a.tags.length, c = 0, h = e.length; h > c; c++) j = e[c],
        ...
        var l = function(a, b) {
            var c, e, f, h, i = 0,
            j = 0;
            for (c = 0, e = d.length; e > c; c++) f = d[c],
            h = !1,
            h = 0 === b.tags.length ? !0 : f.resource.tags.intersects(b.tags),
            h && (j++, (f.status === g.LOADED || f.status === g.ERROR || f.status === g.TIMEOUT) && i++);
            b.callback({
                resource: a.resource,
    ...
    function c(a) {
        ...
        a) {
            ...
            else if ("object" == typeof a) for (var b in a) a.hasOwnProperty(b) && this.all.push(b);
...
"function" == typeof define && define.amd && define("PxLoaderImage", [],
function() {
...
"function" == typeof define && define.amd && define("PxLoaderSound", [],
function() {
...
"function" == typeof define && define.amd && define("PxLoaderVideo", [],
function() {
...
var PxLoaderData = function(a, b, c) {
    ...
    this.start = function(b) {
        e = b,
        d.request.onload = this.onLoad,
        d.request.onerror = this.onError,
        d.request.open("GET", a, !0),
        d.request.send(null)
    },
    this.checkStatus = function() {
...
"function" == typeof define && define.amd && define("PxLoaderData", [],
function() {
    return PxLoaderData
});
    ...
    c.fn.fullpage = function(d) {
        ...
        function Ta(a) {
            ...
            if (!ra(a.target) && ba(b)) {
                ...
                var g = qa(a);
                u && !w && (b = sa(b), E = b.y, L = b.x, a.find(".fp-slides").length && p.abs(M - L) > p.abs(F - E) ? p.abs(M - L) > n.width() / 100 * d.touchSensitivity && (M > L ? k.m.right && e.moveSlideRight() : k.m.left && e.moveSlideLeft()) : d.autoScrolling && p.abs(F - E) > n.height() / 100 * d.touchSensitivity && (F > E ? K("down", g) : E > F && K("up", g)))
            }
        }
        function ra(a, b) {
            b = b || 0;
        ...
        function ta(a, b) {
            for (var d = 0,
            c = a.slice(p.max(a.length - b, 1)), e = 0; e < c.length; e++) d += c[e];
            return p.ceil(d / b)
        }
        function A(a) {
        ...
        function Wa(a) {
            if (d.css3 && d.autoScrolling && !d.scrollBar) xa("translate3d(0px, -" + a.dtop + "px, 0px)", !0),
            d.scrollingSpeed ? ca = setTimeout(function() {
                da(a)
            },
        ...
        function G(a, b) {
            ...
            n || J(b); ! d.loopHorizontal && d.controlArrows && (e.find(".fp-controlArrow.fp-prev").toggle(0 !== f), e.find(".fp-controlArrow.fp-next").toggle(!b.is(":last-child")));
        ...
        function U(a, b) {
            ...
            a === y || c.hasClass("active") ? Fa(c, b) : z(c,
            function() {
                Fa(c, b)
            })
        }
        ...
        function bb() {
            ...
            c.each(r.get(0).className.split(/\s+/),
            function(a, b) {
        ...
        d); (function() {
            ...
            c.each(d.anchors,
            function(a, b) { (c("#" + b).length || c('[name="' + b + '"]').length) && S("error", "data-anchor tags can not have the same value as any `id` element on the site (or `name` element for IE).")
            })
        })();
        c.extend(c.easing, {
        ...
        var O = 0;
        t.on("click touchstart", "#fp-nav a",
        function(a) {
            a.preventDefault();
            a = c(this).parent().index();
            z(c(".fp-section").eq(a))
        });
        t.on("click touchstart", ".fp-slidesNav a",
        function(a) {
            a.preventDefault();
        ...
        }), t.on("mouseleave", d.normalScrollElements,
        function() {
    ...
    a.fn.jScrollPane = function(b) {
        function c(b, c) {
            ...
            function q(a) {
                ...
                d = $ == Z,
                e = a / Z,
                f = -e * (T - Q); (ua != c || wa != d) && (ua = c, wa = d, b.trigger("jsp-arrow-change", [ua, wa, va, xa])),
                t(c, d),
                O.css("top", f),
            ...
            function s(a) {
                ...
                d = ba == aa,
                e = a / aa,
                f = -e * (S - P); (va != c || xa != d) && (va = c, xa = d, b.trigger("jsp-arrow-change", [ua, wa, va, xa])),
                u(c, d),
                O.css("left", f),
            ...
            function F() {
                O.find(":input,a").unbind("focus.jsp").bind("focus.jsp",
                function(a) {
                    x(a.target, !1)
                })
            }
            ...
            function H() {
                ...
                b.attr("tabindex", 0).unbind("keydown.jsp keypress.jsp").bind("keydown.jsp",
                function(b) {
            ...
            function L() {
                var a, b, c, d, e, f = !1;
                R.unbind("touchstart.jsp touchmove.jsp touchend.jsp click.jsp-touchclick").bind("touchstart.jsp",
                function(g) {
...
/*!
 * Generated using the Bootstrap Customizer (http://getbootstrap.com/customize/?id=3980f9aee5b8634471cd)
 * Config saved to config.json and https://gist.github.com/3980f9aee5b8634471cd
 */
...
function(t) {
    ...
    function i(e) {
        return this.each(function() {
            ...
            o = t.extend({},
            s.DEFAULTS, i.data(), "object" == typeof e && e); ! n && o.toggle && /show|hide/.test(e) && (o.toggle = !1),
...
(function(a) {
    ...
    o.prototype = {
        ...
        open: function(b) {
            ...
            n.bgOverlay || (n.bgOverlay = x("bg").on("click" + j,
            function() {
                n.close()
            }), n.wrap = x("wrap").attr("tabindex", -1).on("click" + j,
            function(a) {
            ...
            y("BeforeOpen"),
            n.st.showCloseBtn && (n.st.closeBtnInside ? (w(f,
            function(a, b, c, d) {
            ...
            n.st.enableEscapeKey && s.on("keyup" + j,
            function(a) {
    ...
    var L, M = function() {
        return L === undefined && (L = document.createElement("p").style.MozTransform !== undefined),
        L
    };
    a.magnificPopup.registerModule("zoom", {
        ...
        proto: {
            ...
            _getOffset: function(b) {
                ...
                return M() ? g["-moz-transform"] = g.transform = "translate(" + d.left + "px," + d.top + "px)": (g.left = d.left, g.top = d.top),
                g
            }
        }
    });
...
function(a) {
    ...
    b.prototype.slideHandler = function(a, b, c) {
        ...
        i = this;
        return b = b || !1,
        i.animating === !0 && i.options.waitForAnimate === !0 || i.options.fade === !0 && i.currentSlide === a || i.slideCount <= i.options.slidesToShow ? void 0 : (b === !1 && i.asNavFor(a), d = a, h = i.getLeft(d), g = i.getLeft(i.currentSlide), i.currentLeft = null === i.swipeLeft ? g: i.swipeLeft, i.options.infinite === !1 && i.options.centerMode === !1 && (0 > a || a > i.getDotCount() * i.options.slidesToScroll) ? (i.options.fade === !1 && (d = i.currentSlide, c !== !0 ? i.animateSlide(g,
        function() {
            i.postSlide(d)
        }) : i.postSlide(d)), void 0) : i.options.infinite === !1 && i.options.centerMode === !0 && (0 > a || a > i.slideCount - i.options.slidesToScroll) ? (i.options.fade === !1 && (d = i.currentSlide, c !== !0 ? i.animateSlide(g,
        function() {
            i.postSlide(d)
        }) : i.postSlide(d)), void 0) : (i.options.autoplay === !0 && clearInterval(i.autoPlayTimer), e = 0 > d ? 0 !== i.slideCount % i.options.slidesToScroll ? i.slideCount - i.slideCount % i.options.slidesToScroll: i.slideCount + d: d >= i.slideCount ? 0 !== i.slideCount % i.options.slidesToScroll ? 0 : d - i.slideCount: d, i.animating = !0, i.$slider.trigger("beforeChange", [i, i.currentSlide, e]), f = i.currentSlide, i.currentSlide = e, i.setSlideClasses(i.currentSlide), i.updateDots(), i.updateArrows(), i.options.fade === !0 ? (c !== !0 ? (i.fadeSlideOut(f), i.fadeSlide(e,
        function() {
            i.postSlide(e)
        })) : i.postSlide(e), i.animateHeight(), void 0) : (c !== !0 ? i.animateSlide(h,
        function() {
            i.postSlide(e)
...
var setHeight = function() {
    ...
    // $('.section--full, .person, .recipe__copy .inner').css('min-height', height);
    // $('.recipe').css('height', height);
};

/*
 * Detect scroll direction
 * @param  string  jQuery selector of target element
 */
var detectScrollDirection = function(target) {
    $(target).on('DOMMouseScroll mousewheel',
    function(e) {
...
/*
 * Control a target element's scroll from a specified source object
 * Takes into account OS X "natural" scroll (inverted)
 *
 * @param  string  jQuery selector of the source
 * @param  string  jQuery selector of target object
 * @param  boolean jScrollPane
 *
 */
var scrollTarget = function(source, target) {
    // target to be scrolled
    var tar = $(target);

    // new target to scroll to
    var scrollY = tar.scrollTop();

    // when mousewheel on source
    //$(source).on('mousewheel wheel DOMMouseScroll', function(e){
    $(source).on('mousewheel',
    function(e) {
        ...
        // console.log('e.originalEvent.wheelDeltaY ' + e.originalEvent.wheelDeltaY);
        // // log the values
        ...
        // if scrolling DOWN && e.deltaY === negative value
        // invert them for "normal" scrolling
        //if ( (e.originalEvent.detail > 0 || e.originalEvent.wheelDelta < 0) && (e.deltaY < 0) ) {
        if ((e.originalEvent.deltaY < 0) && (e.deltaY > 0)) {
            // invert values
            scrollY = -(e.deltaY * e.deltaFactor);
            // console.log("inverted scroll " + scrollY);
            // set scrollY to amount scrolled + current scrollTop
            scrollY += tar.scrollTop();
            // console.log("inverted scroll + scrollTop " + scrollY);
        }
        // if scrolling UP && e.deltaY === negative value
        // invert it for "normal" scrolling
        else if ((e.originalEvent.deltaY > 0) && (e.deltaY < 0)) {
            // invert values
            scrollY = -(e.deltaY * e.deltaFactor);
            // console.log("inverted scroll " + scrollY);
            // set scrollY to amount scrolled + current scrollTop
            ...
            // set scrollY to amount scrolled + current scrollTop
            scrollY = ((e.deltaY * e.deltaFactor) + tar.scrollTop());
            // console.log("scroll + scrollTop " + scrollY);
        }

        // if (!jScrollPane){
        // scroll target to scrollY position
        tar.scrollTop(scrollY).scrollLeft(e.deltaX);
        // }
        //else {
        // scroll jScrollPane
        //}
        // console.log("New tar scrollTop " + tar.scrollTop());
    });
};
...
// @codekit-prepend "vendor/pxloader-all.min";
// @codekit-prepend "vendor/jquery.fullPage.min.js";
// @codekit-prepend "vendor/mediaelement-and-player.min.js";
// @codekit-prepend "vendor/jquery.lazyload.min.js";
// @codekit-prepend "vendor/jquery.waypoints.min.js";
// @codekit-prepend "vendor/jquery.mousewheel.min.js";
// @codekit-prepend "vendor/jquery.jscrollpane.min.js";
// @codekit-prepend "vendor/masonry.pkgd.min.js";
// @codekit-prepend "vendor/bootstrap.min.js";
// @codekit-prepend "vendor/jquery.magnific-popup-custom.min.js";
// @codekit-prepend "vendor/slick.min.js";
// @codekit-prepend "vendor/jquery.scrollTo.min.js";
// @codekit-prepend "plugins.js";

// When YouTube API is ready, create the YouTube Player
var videoAbout;
function onYouTubeIframeAPIReady() {
    videoAbout = new YT.Player('video-about', {});
}
...
$(document).ready(function() {
    ...
    var $lazyAbout = $('.section_about .lazy');
    $lazyAbout.lazyload({
        event: 'load.about',
    });

    var $lazyPeople = $('.section_people .lazy');
    $lazyPeople.lazyload({
        event: 'load.people',
    });

    var $lazyCuisineAbout = $('.section_cuisine_about.lazy');
    $lazyCuisineAbout.lazyload({
        event: 'load.cuisine.about',
    });

    var $lazyCuisineRecipes = $('.recipe__image .lazy');
    $lazyCuisineRecipes.lazyload({
        event: 'load.cuisine.recipes',
    });
     ...
     */
    var homeVideo;
    var homeVideoReplay = false;
    var homeVideoMobile;
    var videoHomeSrc = '';
    var videoHomeType = '';

    var $recipeCopy; // jPanelScroll
    var recipeCopyApi; // Store jScrollData for re-initialization
    ...
    // Using Modernizr, detect if browser supports h264
    if (Modernizr.video.h264 !== 'maybe' && Modernizr.video.h264 !== '') {
        // if yes, set preload video url to .mp4 file
        videoHomeSrc = '/media/home.mp4';
        videoHomeType = 'video/mp4';
    } else if (Modernizr.video.webm !== 'maybe' && Modernizr.video.webm !== '') {
        // else, set preload video url to .webm file
        videoHomeSrc = '/media/home.webm';
        videoHomeType = 'video/webm';
    } else {
        videoHomeSrc = '/media/home.mp4';
        videoHomeType = 'video/mp4';
    }
    ...
    function initMobile() {
        ...
        // Hide Preloader on size-up
        $('.preload-wrap').css('display', 'none');

        // Init normal, non-preloaded video
        homeVideoMobile = document.createElement('video');
        homeVideoMobile.setAttribute('poster', '/media/home.jpg');
        homeVideoMobile.setAttribute('src', videoHomeSrc);
        homeVideoMobile.setAttribute('type', videoHomeType);
        homeVideoMobile.setAttribute('id', 'video-home-mobile');
        homeVideoMobile.setAttribute('height', '100%');
        homeVideoMobile.setAttribute('width', '100%');

        var objectFluid = document.createElement('div');
        objectFluid.classList.add('object-fluid');

        $(objectFluid).append(homeVideoMobile);

        // append video
        $('.section_home__video').append(objectFluid);

        // Init MediaElementPlayer
        homeVideoMobile = new MediaElementPlayer('#video-home-mobile', {
        ...
        function initWaypoints() {
            // Waypoints.js Unique Body Classes
            var waypoint_home = new Waypoint({
                element: document.getElementById('home-scroll'),
                handler: function() {
            ...
            var waypoint_about = new Waypoint({
                element: document.getElementById('about-scroll'),
                handler: function() {

                    // pause the video
                    homeVideoMobile.pause();
                },
                offset: 40,
            });

            var waypoint_people = new Waypoint({
                element: document.getElementById('people-scroll'),
                handler: function() {

}
            });

            var waypoint_cuisine = new Waypoint({
                element: document.getElementById('cuisine-scroll'),
                handler: function() {

}
            });

            var waypoint_recipes = new Waypoint({
                element: document.getElementById('recipes-scroll'),
                handler: function() {
            ...
            var waypoint_menu = new Waypoint({
                element: document.getElementById('partners-scroll'),
                handler: function() {
            ...
            var waypoint_menu = new Waypoint({
                element: document.getElementById('credits-scroll'),
                handler: function() {
        ...
        // Init Mobile Menu Opening / Closing funtionality
        var $navMenu = $('.nav-primary__menu-primary');
        var windowHeight = window.screen.height;

        $navMenu.css('height', windowHeight);
        ...
        // open Reservations popup
        $('.open-popup-link').magnificPopup('open');

        // Init scrollTo
        $('.nav-primary__menu-item a').on('click',
        function(event) {
            event.preventDefault();

            closeMobileNav();

            if (event.target.hash !== '#reservations') {

                // remove disabled from real active nav item
                $('.nav-primary__menu-item.active.disabled').removeClass('disabled');

                // add active class to <li>
                // remove class from sibiling <li>
                // MFP open() / close() handles Reservations link highlighting
                $(this).closest('li').addClass('active').siblings().removeClass('active');

                var $target = $(event.target.hash + '-scroll');
                $.scrollTo($target, {
                    duration: 750,
                    offset: -40,
                });

                // If reservations popup is open, close it
                $.magnificPopup.close();
            }
        });

        // Partners Deeplink Mobile
        $(window).on('load',
        function() {
            if (!document.location.hash) {
                return false;
            }
            if (document.location.hash === '#partners') {
                $(window).scrollTo('#partners-scroll', {
                    duration: 500
                });
            }
        });
    ...
    function preloadInit() {
        ...
        // preload the video
        homeVideo = preloader.addVideo(videoHomeSrc);

        // when video is loaded...
        preloader.addCompletionListener(function(e) {

            // set video attributes
            homeVideo.setAttribute('poster', '/media/home.jpg');
            homeVideo.setAttribute('id', 'video-home');
            homeVideo.setAttribute('type', videoHomeType);
            homeVideo.setAttribute('height', '100%');
            homeVideo.setAttribute('width', '100%');

            // append video
            $('.section_home__video').append(homeVideo);

            // init MediaElement.js
            homeVideoInit();

            // Only Play video if on home page
            // if ( (document.location.hash === "#home" || document.location.hash === "") && (window.innerWidth > 1024) ) {
            //     homeVideo.play();
            // }
            // On initial page load, go to next section when video ends
            $(homeVideo.media).one('ended',
            function() {
                $.fn.fullpage.moveSectionDown();
            });

            // append loaded class to body
            // initialize preloader fadeout
            $('body').toggleClass('loaded');

            // Remove preloader div
            // Delay 1500ms to match preloader fadeout
    ...
    function homeVideoInit() {
        homeVideo = new MediaElementPlayer('#video-home', {
            // initial volume when the player starts
            startVolume: 1.0,
            // useful for <audio> player loops
            loop: false,
            ...
            success: function(mediaElement, domElement) {
                ...
                // add custom homeVolume button
                var $homeVolume = $('.home_volume');

                // append homeVolume button to video player
                $('.section_home__video .mejs-volume-button').append($homeVolume);

                $('.mejs-volume-button').hover(function() {
                    $homeVolume.css('display', 'block');
                },
                function() {
                    $homeVolume.css('display', 'none');
                });
                ...
                function(e) {
                    ...
                    // unmute and set the video volume
                    homeVideo.setMuted(false);
                    homeVideo.setVolume(volume);

                    // append class to parent for main volume icon update
                    if (volume === 1) {
                        $(this).closest('.mejs-volume-button').addClass('volume--high').removeClass('volume--medium volume--low');
                    } else if (volume === 0.5) {
                        $(this).closest('.mejs-volume-button').addClass('volume--medium').removeClass('volume--high volume--low');
                    } else if (volume === 0.25) {
                        $(this).closest('.mejs-volume-button').addClass('volume--low').removeClass('volume--high volume--medium');
                    }
    ...
    //
    function jPanelInit() {
        $recipeCopy = $('.recipe__instructions .inner');

        $recipeCopy.jScrollPane({
            showArrows: false,
            verticalGutter: 100,
            hideFocus: true,
        });

        $recipeCopy.each(function() {
            var jpsApi = $(this).data('jsp');
            var throttleTimeout;

            var debounceReInitJps = debounce(function() {
                if (!throttleTimeout) {
            ...
            $(window).on('resize', debounceReInitJps);
        });
    } // jPanelInit()
    ...
    function initFullpage() {
        ...
        // Init jPanelScroll, not on iPads
        ...
        var firstPersonIsActive = false,
        lastPersonIsActive = false,
        firstRecipeIsActive = false,
        lastRecipeIsActive = false,
        allowAllScrolls = false;

        var imagesPeopleLoaded = false,
        imagesRecipesLoaded = false;
        ...
        $('#fullpage').fullpage({
            ...
            normalScrollElements: '.recipe__instructions .inner, .gallery_items, .jspContainer, .jspPane',
            ...
            afterLoad: function(anchorLink, index) {
                ...
                if (index === HOME && document.location.hash === "#home" && homeVideoReplay) {
                    homeVideo.play();
                }

                if (index === PEOPLE) {
                    firstPersonIsActive = $('.person:first-child').hasClass('active');
                    lastPersonIsActive = $('.person:last-child').hasClass('active');
                } else if (index === RECIPES) {
                    firstRecipeIsActive = $('.recipe:first-child').hasClass('active');
                    lastRecipeIsActive = $('.recipe:last-child').hasClass('active');
                }

                // Lazy Load Images
                // load the current and next section's images
                if (index === HOME) {
                    $lazyAbout.trigger('load.about');
                } else if (index === ABOUT) {
                    $lazyAbout.trigger('load.about');
                    $lazyPeople.trigger('load.people');
                } else if (index === PEOPLE) {
                    $lazyPeople.trigger('load.people');
                    $lazyCuisineAbout.trigger('load.cuisine.about');
                } else if (index === CUISINE) {
                    $lazyCuisineAbout.trigger('load.cuisine.about');
                    $lazyCuisineRecipes.trigger('load.cuisine.recipes');
                } else if (index === RECIPES) {
                    $lazyCuisineRecipes.trigger('load.cuisine.recipes');
                }
                ...
                if (index === GALLERY) {
                    if (tumblr_offset === 0) {
                        retrieveTumblrPosts(tumblr_offset);
                    }
                }
            ...
            afterSlideLoad: function(anchorLink, index, slideAnchor, slideIndex) {
                ...
                if (index === PEOPLE) {
                    firstPersonIsActive = (slideIndex === 0);
                    lastPersonIsActive = (slideIndex === 8);
                } else if (index === RECIPES) {
                    firstRecipeIsActive = (slideIndex === 0);
                    lastRecipeIsActive = (slideIndex === 5);
                }
            },
            ...
            onLeave: function(index, nextIndex, direction) {
                ...
                if (index === ABOUT && videoAbout && (videoAbout.getPlayerState() === 1 || videoAbout.getPlayerState() === 3)) {
                    videoAbout.pauseVideo();
                }

                // Lazy Load Images
                // load the section's images when going to that section
                if (nextIndex === ABOUT) {
                    $lazyAbout.trigger('load.about');
                } else if (nextIndex === PEOPLE && !imagesPeopleLoaded) {
                    $lazyPeople.trigger('load.people');
                    imagesPeopleLoaded = true;
                } else if (nextIndex === CUISINE) {
                    $lazyCuisineAbout.trigger('load.cuisine.about');
                } else if (nextIndex === RECIPES && !imagesRecipesLoaded) {
                    $lazyCuisineRecipes.trigger('load.cuisine.recipes');
                    imagesRecipesLoaded = true;
                }
                ...
                if (nextIndex === HOME && direction === 'up') {
                    // console.log('headed home');
                    $menu.removeClass('top');

                    // Restart and play the Home Video
                    homeVideoReplay = true;
                    homeVideo.setCurrentTime(0);

                } else if (index === HOME && direction === 'down') {
                    $menu.addClass('top');

                    // Reset homeVideoReplay
                    homeVideoReplay = false;
                }

                if (index === PEOPLE && direction === 'up') {
                ...
                if (index === PEOPLE && direction === 'down') {
                    // Are we on the last person slide?
                    // If not, advance the slide and deny the scroll
                    if (!lastPersonIsActive) {
                ...
                if (index === RECIPES && direction === 'down') {
                    // Check whether the recipe is scrolled down to the
                    // bottom; deny the scroll if not.
                    // var scroll = checkScroll({
                    //     target: '.recipe.active .recipe__instructions .inner'
                    // });
                    // if (!scroll.bottom) {
                    //     return false;
                    // }
                    // Are we on the last recipe? If not, deny the scroll and
                    // move to the next slide.
                    if (!lastRecipeIsActive) {
                ...
                if (index === RECIPES && direction === 'up') {
                    // Check whether the recipe is scrolled up to the
                    // top; deny the scroll if not.
                    // var scroll2 = checkScroll({
                    //     target: '.recipe.active .recipe__instructions .inner'
                    // });
                    // if (!scroll2.top) {
                    //     return false;
                    // }
                    // Are we on the first recipe? If not, deny the scroll and
                    // move to the previous slide
                    if (!firstRecipeIsActive) {
                        $.fn.fullpage.moveSlideLeft();
                        return false;
                    }
                }

                if (nextIndex === RECIPES && $recipeCopy && $recipeCopy.data('jsp')) {
                    // Reinit jScrollPane
                    $recipeCopy.each(function() {
                        recipeCopyApi = $(this).data('jsp');
                        recipeCopyApi.reinitialise();
                    });
                }

                if (index === GALLERY && direction === 'up') {
                    // Check whether the gallery is scrolled up to the
                    // top; deny the scroll if not.
                    var scroll8 = checkScroll({
                        target: '.gallery .inner'
                    });
                ...
                // if (index === GALLERY && direction === 'down') {
                //     // Check whether the gallery is scrolled down
                //     // to the bottom; load more posts if it is
                //     var scroll9 = checkScroll({
                //         target: '.gallery .inner'
                //     });
                //     if (scroll9.bottom) {
                //         console.log("down");
                //         if(tumblr_offset < 0) tumblr_offset = 0;
                //         console.log(tumblr_offset);
                //         if(!no_more_tumblr_posts) {
                //             retrieveTumblrPosts(tumblr_offset);
                //         } else {
                //             tumblr_offset = 0;
                //         }
        ...
        //
        // Init Slick Slider for section subnav
        //
        $('.people .nav_inner_list').slick({
            speed: 1300,
            arrows: false,
            centerMode: true,
            infinite: false,
        ...
        $('.cuisine_recipes .nav_inner_list').slick({
            speed: 1500,
            arrows: false,
            centerMode: true,
            infinite: false,
        ...
        //
        //  Handle Receipe Next link clicks
        //
        $('.recipe-link a').on('click',
        function(e) {
            e.preventDefault();
            if (!lastRecipeIsActive) {
                $.fn.fullpage.moveSlideRight();
        ...
        //
        $('.nav-primary__menu-item > a').on('click.burnt.lg',
        function(event) {
            var $menu = $('.nav-primary');
            var dest = event.target.hash;
        ...
        // 
        // Capture scroll on recipe image
        //
        // $('.recipe').each(function() {
        //     console.log(this);
        //     var instructions = this.children[0].children[0];
        //     var image = this.children[1];
        //     scrollTarget(image, instructions);
        // });
    } // init desktop
    ...
    function reactToHash(event) {
        ...
        var slide = hashparts[1];

        // Sync Slick Slider subnavigation with current slide
        if (section === '#people' && slide) {
            $('.people .nav_inner_list').slick('slickGoTo', slide);
        }

        if (section === '#recipes' && slide) {
            $('.cuisine_recipes .nav_inner_list').slick('slickGoTo', slide);
        }

        if (section === '#menu' || section === '#recipes') {
            section = '#cuisine';
        }

        // If hash isn't blank, then add/remove active classes
        if (hash !== '') {
            $('a[href="' + hash + '"], a[href=' + section + ']').closest('li').addClass('active');
            $('a:not([href="' + hash + '"], [href=' + section + '])').closest('li').removeClass('active');
        }

        // Deeplink to Partners section
        if (hash === '#partners') {
            // Go To About Section
            $.fn.fullpage.moveTo(2);

            // Open Partners section after 1s
            var openPartners = setTimeout(function() {
                $('#partners-content').collapse('show');
            },
            1000);
        }
     ...
     */
    $('.section_about .collapse').collapse({
        toggle: false
    });

    $('.section_about .collapse').on('show.bs.collapse',
    function() {
        $(this).closest('.tab').css('z-index', 10);
        // $lazyAbout.trigger('load.about');
    }).on('hidden.bs.collapse',
    function() {
        $(this).closest('.tab').css('z-index', 0);
    });
    ...
    // Sub-Recipe Sections
    // Bootstrap Collapse.js
    var $recipeParts = $('.recipe__parts .collapse');
    $recipeParts.collapse({
        toggle: false
        // parent: $('#recipe_one__accordian') // enables Accordian-style behavior
    });

    // If jPanelScroll has been initialized,
    // Reinitialize it every time a BS panel opens or closes
    $recipeParts.on('shown.bs.collapse hidden.bs.collapse',
    function() {

        if ($recipeCopy && $recipeCopy.data('jsp')) {
            // Reinit jScrollPane
            $recipeCopy.each(function() {
                recipeCopyApi = $(this).data('jsp');
                recipeCopyApi.reinitialise();
    ...
    $('.open-popup-link').magnificPopup({
        ...
        callbacks: {
            open: function() {
                ...
                // add active.disabled class to item
                $('.nav-primary__menu-item.active').addClass('disabled');

                // add .active class to el parent <li>
                $(this.currItem.el[0]).closest('li').addClass('active');

                // if Home and video is playing, pause it
                if ((document.location.hash === "#home" || document.location.hash === "") && (window.innerWidth > 1024) && (homeVideo.media.currentTime !== 0)) {
                    // if video is playing, pause video
                    homeVideo.pause();
                }
            },
            close: function() {
                // Remove disabled class from real current item
                $('.nav-primary__menu-item.active.disabled').removeClass('disabled');

                // Remove active class from Reservations
                $(this.currItem.el[0]).closest('li').removeClass('active');
            }
        }
    });
    ...
    //
    var tumblr_offset = 0,
    no_more_tumblr_posts = false,
    tumblr_tag = "all",
    first_tumblr_request = true; // only init Masonry on first request
    var tumblr_posts_per_request = 10,
    // tumblr_api_key = "eErxTTSUqTvaVPG1GnvFXP0PQcRVUu1Yqo0nCD0QpHrpfEQQm3",
    // tumblr_api_root_path = "https://api.tumblr.com/v2/blog/stagingtest888.tumblr.com/",
    tumblr_api_key = "EIExoNvYVTC7fbFB5zF5BDHIC94mfX3zEkB8FLKjsBz3IVkjba",
    // prod
    tumblr_api_root_path = "https://api.tumblr.com/v2/blog/burntmovie2015.tumblr.com/",
    // prod
    tumblr_total_posts;

    //
    // Set grid variable for Masonry
    //
    var $grid = $('.grid');
    ...
    // Detects bottom of .section_gallery .inner
    //
    var debouncedScroll = debounce(function() {
        if (!no_more_tumblr_posts && $(this).scrollTop() + $(this).innerHeight() >= this.scrollHeight) {
            $('#loader').addClass('active');
            retrieveTumblrPosts(tumblr_offset);
        }
    },
    100);
    $('.section_gallery .inner').scroll(debouncedScroll);

    //
    // Get Tumblr Posts
    //
    function retrieveTumblrPosts(tumblr_post_offset) {
        //console.log('tumblr retrieve posts triggered');
        // If there are Tumblr posts to retrieve
        if (!no_more_tumblr_posts) {

            no_more_tumblr_posts = true;

            // Build the request for Tumblr posts
            var request;
            request = tumblr_api_root_path + "posts?callback=?&api_key=" + tumblr_api_key + "&limit=" + tumblr_posts_per_request + "&offset=" + tumblr_post_offset;
            if (tumblr_tag !== "all") {
                request += "&tag=" + tumblr_tag;
            }
            ...
            function(data) {
                ...
                // Activate the loader indicator
                $('#loader.active').removeClass('active');

                // Get total posts data to know when we are at the end
                if (!tumblr_total_posts) {
                    tumblr_total_posts = data.response.total_posts;
                    // console.debug("Total Posts", tumblr_total_posts);
                }

                // console.debug("TUMBLR Data",data);
                // console.debug("TUMBLR Data",data.response.posts.length);
                //
                // Handle each post
                //
                ...
                function(i, item) {
                    ...
                    switch (item.type) {
                    case "photo":
                        handleImageContent(item);
                        break;
                    case "video":
                        handleVideoContent(item);
                        break;
                    default:
                        handleDefaultContent(item);
                    }
                });

                // There are Tumblr posts left
                no_more_tumblr_posts = false;

                // If there are Tumblr posts left
                if ((data.response.posts.length === tumblr_posts_per_request) && (tumblr_offset !== tumblr_total_posts)) {
                    // Set tumblr_offset to the "next" set of posts
                    tumblr_offset += tumblr_posts_per_request;
                } else {
                    no_more_tumblr_posts = true;
                }

                //
                // If this is the first Tumblr request
                // Initialize Masonry
                //
                if (first_tumblr_request === true) {
                    ...
                    first_tumblr_request = false;
                }

                // Manually initialize Masonry layout
                // every time getJSON is called
                $grid.masonry();
    ...
    function handleFilter($el) {
        // only do something if filter is changing
        if ($el.data("tag") !== tumblr_tag) {
            // set the new active tag
            tumblr_tag = $el.data("tag");

            // Reset the Grid
            resetTumblrGrid();

            // Highlight current filter
            $(".tumblr-filter.active").removeClass("active");
            $el.addClass("active");

            // Get requested Tumblr posts
            retrieveTumblrPosts(tumblr_offset);
    ...
    //
    // Reset the Tumblr Grid
    //
    function resetTumblrGrid() {
        // reset the offset
        tumblr_offset = 0;
    ...
    // Handle Image Posts
    //
    function handleImageContent(item) {

        var image;
        var alt_sizes = item.photos[0].alt_sizes.length;
        var aspectRatio;
        var $newItem;

        if (alt_sizes > 3) {
            image = item.photos[0].alt_sizes[2];
        } else if (alt_sizes > 2) {
            image = item.photos[0].alt_sizes[1];
        } else {
            image = item.photos[0].alt_sizes[0];
        }
        ...
        if (image.width !== image.height) {
            $newItem = $('<div class="grid-item grid-item--width-2"><div class="grid_item__inner" style="padding-bottom:' + aspectRatio + ';"><a target="_blank" href="' + item.post_url + '" ><img src="' + image.url + '" /></a></div></div>');
        } else {
            $newItem = $('<div class="grid-item grid-item--width-1"><div class="grid_item__inner" style="padding-bottom:100%;"><a target="_blank" href="' + item.post_url + '" ><img src="' + image.url + '" /></a></div></div>');
        }
    ...
    // Handle Video Posts
    //
    function handleVideoContent(item) {
        var aspectRatio = (item.thumbnail_height / item.thumbnail_width) * 100 + '%';
        var $newItem;

        //console.debug("VIDEO", item);
        if (item.thumbnail_width !== item.thumbnail_height) {
            $newItem = $('<div class="grid-item grid-item--width-3"><div class="grid_item__inner" style="padding-bottom:' + aspectRatio + ';"><a target="_blank" href="' + item.post_url + '" ><img src="' + item.thumbnail_url + '" /></a></div></div>');
        } else {
            $newItem = $('<div class="grid-item grid-item--width-3"><div class="grid_item__inner" style="padding-bottom:100%;"><a target="_blank" href="' + item.post_url + '" ><img src="' + item.thumbnail_url + '" /></a></div></div>');
        }
    ...
    // Handle Other Posts
    //
    function handleDefaultContent(item) {
        // console.debug("DEFAULT", item);
    }
    ...
    // Destroy desktop version of website when window resizes < 1024
    // Destroy FullPage.js
    // Destroy jScrollPane
    // Unbind .nav listeners
    // Unbind window hashchange listeners
    // Remove homeVideo
    // Init Mobile
    var destroyDesktop = debounce(function() {

        // Set target width 
        targetWidth = window.innerWidth;
        var orientation = window.orientation;

        // if currentWidth >= 1024 && targetWidth < 1024
        // user resized their browser
        if (currentWidth >= 1024 && targetWidth < 1024 && typeof orientation === 'undefined') {

            // destroy FullPage.js
            $.fn.fullpage.destroy('all');

            // if user resized window on desktop
            // destroy jScrollPane
            if ($recipeCopy.data('jsp')) {
                $recipeCopy.each(function() {
            ...
            // Unbind .nav listeners
            $('.nav-primary__menu-item > a').off('click.burnt.lg');

            // Unbind window hashchange listeners
            $(window).off('hashchange', reactToHash);

            // Stop playing homeVideo
            homeVideo.pause();
            var homeVideoPlayer = $('#video-home')[0].player;
            homeVideoPlayer.remove();
            $('#video-home').remove();

            // Init Mobile Functionality
            initMobile();

            currentWidth = window.innerWidth;
        }

        // if window has been resized down from desktop
        // the resized up, display a "refresh" message
        if (currentWidth <= 1024 && targetWidth > 1024 && reload === false && typeof orientation === 'undefined') {
            var $reload = $('<div>');
            $reload.addClass('reload-message absolute-full').html('<p class="title--alt  center-vertical">Please reload this page</p>');
